...
class OandaPosition(object):
    ...
    def __str__(self):
        ...
        items.append('- Update: {}'.format(self.updt))
        items.append('--- Position End')
        return '\n'.join(items)

    def __init__(self, size=0, price=0.0, dt=None):
        self.size = size
        if size:
        ...
        self.updt = dt or datetime.utcnow()
    ...
    def update(self, size, price, dt=None):
        ...
        '''
        self.updt = dt or datetime.utcnow()  # record datetime update (datetime.datetime)
